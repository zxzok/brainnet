...
- **Input**: `roi_timeseries_id: str`, `method: "pearson" | "partial"`
...
- **Wraps**: `DynamicAnalyzer.analyse`
- **Side effect**: Stores full `AnalysisArtifacts` in SessionStore under `strategy_id`

#### `extract_features`
- **Purpose**: Extract comprehensive features from analysis results
- **Input**: `strategy_id: str`
- **Output**: `feature_names: list[str]`, `feature_values: list[float]`, `feature_types: list[str]`, `summary_stats: dict`
- **Wraps**: Feature extraction logic from `persist_analysis_features`
...
```python
async def run_orchestrator(session: SessionStore, user_message: str):
    ...
    while True:
        ...
        for event in response:
            if event is text_delta:
                yield SSEEvent("text_delta", event.text)
            elif event is tool_use:
                yield SSEEvent("tool_start", tool_name, tool_input)
                result = execute_tool(session, tool_name, tool_input)
                yield SSEEvent("tool_result", result_summary)
                # Append tool_use and tool_result to messages
                # Continue loop for next Claude turn
...
- Session history capped at ~50K tokens
- When approaching limit, older tool results are summarized (full metrics → key metrics only)
- User warned when nearing the limit
...
// Parse SSE events, update DOM progressively
```
...
| `tools.py` | Tool JSON schemas + Python implementations | ~350 |
...
- `anthropic` (Anthropic Python SDK) — added as optional dependency under `[llm]` extra
