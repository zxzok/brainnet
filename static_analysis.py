...
class StaticAnalyzer:
    ...
    def compute_graph_metrics(self, conn_matrix: ConnectivityMatrix) -> GraphMetrics:
        ...
        return GraphMetrics(node_metrics=node_metrics, global_metrics=global_metrics)
            ...
            if k_i < 2:
                clustering[i] = 0.0
                continue
            # compute sum of triad weights
            # sum_{j<k} (w_ij*w_jk*w_ik)^(1/3)
            tri_sum = 0.0
        ...
        efficiency = self._compute_global_efficiency(abs_mat)
        # assemble metrics
        node_metrics = {
            'degree': degrees,
            'clustering': clustering,
    ...
    def _compute_global_efficiency(self, abs_mat: np.ndarray) -> float:
        """Compute the global efficiency of a weighted undirected graph.
        ...
        ----------
        abs_mat : np.ndarray
            Non‑negative symmetric adjacency matrix (absolute weights).
        ...
        """
        N = abs_mat.shape[0]
        # distance matrix: set zero weights to inf
        with np.errstate(divide='ignore'):
            dist = 1.0 / abs_mat
        dist[abs_mat == 0] = np.inf
        np.fill_diagonal(dist, 0.0)
        # Floyd‑Warshall algorithm for all‑pairs shortest paths
        # copy to avoid modifying dist
        D = dist.copy()
        for k in range(N):
            # update distances through intermediate k
            # broadcasting ensures vectorised min
